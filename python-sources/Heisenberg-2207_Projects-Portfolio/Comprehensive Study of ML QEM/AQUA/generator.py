...
class QuantumGenerator:
    ...
    def generate_data(self, output_format="csv"):
        ...
        for depth in range(*self.depth_params):
            ...
            for _ in tqdm(range(self.circuits_per_depth), desc=f"Circuits at depth {depth}", leave=True, position=0):
                ...
                simulator = QuantumCircuitSimulator(compiled_circ, self.n_qubits, shots=self.shots, iterations=1, use_ibm=False)
                ideal_circuit, noisy_circuit, ideal_expectation, noisy_expectation = simulator.run_simulation()
                # entry["IdealCircuit"] = ideal_circuit
                entry["NoisyCircuit"] = noisy_circuit
                entry["IdealExpectation"] = ideal_expectation
                entry["NoisyExpectation"] = noisy_expectation

                # Run ZNE simulation using ZNESimulator.
                zne_simulator = ZNESimulator(circuit=compiled_circ, n_qubits=self.n_qubits, shots=self.shots,
                                             scale_factors=self.scale_factors, noise_model=simulator.noise_model)
                # run_simulation is assumed to accept parameters return_ieal and return_noisy.
                ...
                for conv_type in self.conversion_type:
                    converter = CircuitConverter(max_gate_attributes=3, bin_min=-3.14, bin_max=3.14, bin_size=0.5, mode='bin')
                    dataset_conv = converter.convert(noisy_circuit, conversion_type=conv_type)
...
if __name__ == "__main__":
    ...
    qgen = QuantumGenerator(
        n_qubits=5,
        depth=(4, 10, 2),         # Depth values: 4, 6, 8
